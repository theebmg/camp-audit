...
naming debt from the Postgres cutover (`Camp Sychar Audit (Postgres
preview)` as the tab title, `Camp Sychar (pg)` in the sidebar) plus a
few leftover dev-facing strings that read as scaffolding once the app went
from "preview" to "the only app."

**Naming.** Ben's call: the app is his own working tool, not board-facing,
so no "CMMS" jargon in his own tab. New name **"Sychar Operations"**
everywhere the product name appears (browser tab title, sidebar brand, main
`<h1>`, login screen title, PDF/email report footer in `reportRender.js`'s
`htmlShell()`). "Audit" was dropped everywhere it named the *app* — it's
kept everywhere it names the *feature* (Start Audit, audit sessions,
`renderAudit()`, etc.), since the app does more than audits now.
**Note:** `reportRender.js`'s footer is shared by every emailed report
(Board Report, Forward Focus, Work Performed, Deferred Backlog) and by
plain request-status emails to external requesters via
`renderPlainEmailHtml()` — "Generated by Sychar Operations" now shows on
those too, not just internal screens. Named report titles/subjects
themselves (`CAMP SYCHAR — BOARD REPORT`, email subject lines, the request
portal's "Camp Sychar Maintenance" signature) were left untouched — not
named in this pass, and arguably correct to keep separate: those are
external/board-facing, this app's own chrome isn't.

**Deleted:** the always-on `.pg-banner` div ("🚧 Postgres migration preview
— separate database from the main app, safe to click around") that showed
on every single page load — pure leftover scaffolding from when this was a
parallel preview build, not a real notice. Its CSS rule went with it. Two
internal comments referencing "this preview build" and "scoped to /next/
only" were reworded to describe current reality (this file, not a preview
build; this page, not `/next`) — `/next` itself still exists as a 301
redirect to root (unrelated, untouched, still correct).

**Empty-state copy** rewritten to say what to do next, scoped to
high-traffic screens only (Work Orders, Requests, Notes — both the
top-level Notes view and the per-asset Field Notes card, Locations, Assets-
in-Location, Inbox) — admin-only empty states (Causes, Sub-Areas,
Attachment Roles, Checklists, Templates, Component Types, Property Fields,
etc.) were deliberately left as plain "No X yet." per Ben's call, not
padded. The Inbox's empty state also lost its "(once configured)"
hedge on email — Mailgun ingest is live as of yesterday's session (see the
Mail Ingest runbook below) — and now names the actual address,
`photos@cmms.fracturedrv.com`. One state (`renderAssetsInLocation`'s "No
assets in this location") got the most scrutiny: **there is no "create
asset" flow anywhere in the app's own navigation** — assets are only ever
created via the searchable `mountAssetCombobox` component's "+ Add new
asset" option, reachable from Start Audit or a work order's asset picker,
never as a standalone action. The empty-state copy points there since it's
the only real path; worth knowing if a dedicated "create asset" entry point
is ever wanted.

**`USER_GUIDE.md`**: turned out to already be current on job lines,
attachments, quotes, and splitting (it wasn't written before those
features, contrary to the assumption going into this pass) — the only
actually-stale material was the `/next` URL as the guide's primary
bookmark-this address, the entire "two web addresses" closing section
(claiming root was "an older interface... isn't where new work should
happen" — backwards since the NocoDB/Atlas teardown), and one "(once that's
turned on)" hedge on email ingest. All three fixed; nothing else in the
guide needed correction. Retitled to match: "Sychar Operations — User
Guide."
